...
function goTo(id){
  ...
  next.classList.add('active');currentScreen=id;next.scrollTop=0;
...
function setTab(group,tab,btn){
  ...
  document.querySelectorAll(`[id^="${prefix}"]`).forEach(el=>{el.style.display=el.id===prefix+tab?'':'none';});
  ...
  if(group==='detail'){
    ...
    if(tab==='info'){const room=rooms.find(r=>r.id===activeRoomId);if(room)renderDetailInfo(room);}
  }
}
